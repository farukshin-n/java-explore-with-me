...
import static ru.practicum.ewmservice.util.PageRequestUtil.getPageRequest;
...
public class CommentServiceImpl implements CommentService {
    ...
    @Override
    public List<CommentDto> getAllCommentsByEvent(Long eventId, CommentSort sort, int from, int size) {
        return commentRepository.findCommentsByEvent_Id(eventId, sort, getPageRequest(from, size)).stream()
                .map(CommentMapper::toDto)
                .collect(Collectors.toList());
    }

    @Override
    public List<CommentDto> getAllCommentsByUser(Long userId, CommentSort sort, int from, int size) {
        return commentRepository.findCommentsByAuthor_Id(userId, sort, getPageRequest(from, size)).stream()
                .map(CommentMapper::toDto)
                .collect(Collectors.toList());
    }

    @Override
    public List<CommentDto> getAllCommentsSortedByUpdated(CommentSort sort, int from, int size) {
        return commentRepository.findAllCommentsSortedByUpdated(sort, getPageRequest(from, size)).stream()
                .map(CommentMapper::toDto)
                .collect(Collectors.toList());
    }
    ...
    @Override
    @Transactional
    public CommentDto unhide(Long eventId, Long commentId) {
        final Comment comment = findCommentByIdAndEventId(commentId, eventId);
        ...
        final Comment updatedComment = commentRepository.save(comment);
        log.info("Admin unhide comment with id={} to event with id={}.",
                updatedComment.getId(), updatedComment.getEvent().getId());
